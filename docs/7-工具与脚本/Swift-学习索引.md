...
| `.onScrollGeometryChange(for:of:action:)` (macOS 15+) | `RepoAIWindowContentView` 对话段一次 transform 同时取 `offsetY + distanceFromBottom`（用 `ScrollFollowTailMetrics: Equatable` 包两个量），既给"跟随尾部"用、又给"顶部下拉 overscroll 切面板"用，避免一个 ScrollView 上挂两个 modifier | "SwiftUI onScrollGeometryChange ScrollGeometry macOS 15" |
| `ScrollPhase` (macOS 15+) | `ScrollTailController.lastPhase` 持久化最新相位，跨"跟随尾部"与 overscroll 两个滚动驱动行为共享同一份 phase tracking 避免漂移；switch 时必须写 `@unknown default` 兜底 | "SwiftUI ScrollPhase enum tracking interacting decelerating animating" |
| `ScrollGeometry.contentSize / contentOffset / containerSize` (macOS 15+) | `ScrollFollowTail` 用 `contentSize.height - contentOffset.y - containerSize.height` 算"离底距离"，配合 1pt/8pt 双阈值滞回区判断是否要关闭/恢复跟随（"一滚就停 + 几乎到底就恢复"语义）| "SwiftUI ScrollGeometry contentSize containerSize" |
| `.pointerStyle(_:)` (macOS 15+) | `CommitHashLink` 用 `.pointerStyle(.link)` 显式把鼠标变成手型指针——SwiftUI `Link(destination:label:)` **closure-based init** 会绕过 `_LinkLabel` 私有装饰链，hover 下划线 / 手型 cursor / tint 着色全失效，需要手动重补；比 AppKit `NSCursor.pointingHand.push()/pop()` 安全（cursor 栈由 SwiftUI 系统管理生命周期不会卡住） | "SwiftUI pointerStyle PointerStyle link cursor macOS 15" |
| `Link(destination:label:)` closure-based init 装饰链丢失 | `Link("text", destination:)` string-based init 内部用私有 `_LinkLabel` 自动渲染 hover 下划线 + 手型指针 + tint；一旦改用 `Link(destination:){ Text(...) }` closure-based 形式自定义 label，**整套装饰链丢失**，只剩 tint 着色和点击行为。`CommitHashLink` 必须用 closure-based（因父级 footer 有 `.foregroundStyle(.secondary)` 会染灰 string-based 的 _LinkLabel），所以手动补 `@State isHovered` + `.onHover` + `Text.underline(isHovered)` + `.pointerStyle(.link)` 三件套 | "SwiftUI Link closure label underscore underline cursor macOS" |
...
| "Command-line preferences" | Apple 内置约定：**Xcode Scheme launch args `-Key Value` 自动注册到 `UserDefaults.standard`**。`DebugFlags` 利用这点，不写一行解析代码就能用 Scheme 切调试开关。详见 `docs/7-工具与脚本/调试工具.md` §1.1(详见 `docs/2-产品/需求讨论/正式方案/Pro付费墙验证清单.md`) | "NSUserDefaults command-line preferences" |
...
| Scheme → Arguments → Arguments Passed On Launch | 给 App 传命令行参数；`-Key Value` 形式会自动进 UserDefaults。`DebugFlags` 用这个开调试 overlay，详见 `docs/7-工具与脚本/调试工具.md` §3.1 | "Xcode scheme arguments launch" |
...
| "actor 和 class 区别？" | §1.1(详见 `docs/2-产品/需求讨论/正式方案/Pro付费墙验证清单.md`) + §2 → "Swift actors data race" |
| "Task cancel 怎么生效？" | §2 `Task.cancel()` → 看 `HomeViewModel.reloadItems` 第 122-185 行 |
| "为什么协议都要 Sendable？" | §1.5(详见 `docs/2-产品/需求讨论/正式方案/Pro付费墙验证清单.md`) + §2 → "Swift Sendable concurrency checking" |
| "any vs some 哪个用哪个？" | §1.5(详见 `docs/2-产品/需求讨论/正式方案/Pro付费墙验证清单.md`) `any/some` → Swift Evolution SE-0335 |
...
| "什么是 capture list 丢 default value？" | §1.6(详见 `docs/2-产品/需求讨论/正式方案/Pro付费墙验证清单.md`) + 看 `GitHubAPIClientTests.swift` 第 24-39 行（解决方案是用 file-level free function） |
| "Swift 怎么读配置文件 / 调试开关怎么加？" | §10 `UserDefaults` / "Command-line preferences" + §11 launch arguments → **完整使用指南见 `docs/7-工具与脚本/调试工具.md`**（A/B/C 三种切换方式 + 双保险 + 新增 SOP） |
