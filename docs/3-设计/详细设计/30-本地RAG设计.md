...
- 会话历史按当前模型的统一 Context Budget 计算历史份额；“已摘要 + 尚未摘要原文”达到历史预算 85% 才触发语义压缩。压缩后至少保留最近 3 轮原文，较早内容由持久化摘要替代；压缩失败使用受限本地摘要但不删除原始消息。最终 Prompt 仍按剩余 token 从近到远稳定裁剪，避免长会话无限增长。
...
13. 工作台与知识库浏览器用 `RAGIndexStatusProjection` 统一索引空态、仓库/向量覆盖和问题计数口径；该类型只能是从 SQL 聚合结果构造的纯值读模型。两个窗口的刷新任务、仓库选择、分页、问答、问题抽屉和重建进度仍由各自 ViewModel 管理，禁止共享可观察 UI 状态机。
...
- 多轮历史未达到模型历史预算 85% 时可保留超过 3 轮原文；达到水位后压缩较早内容，并至少保留最近 3 轮。摘要受模型窗口派生预算约束，原始消息始终持久化。
