...
- bulk 来源目录存入 `weekly_bulk_sources`，通用条目和置顶字段由 `v13-weekly-multi-source` 追加迁移保存；
...
`skills/starcat-weekly-import` 只负责文本解析、搜索核验、用户确认和整批提交。脚本默认 dry-run，只有显式 `--confirm` 才提交，不记录 admin key。
